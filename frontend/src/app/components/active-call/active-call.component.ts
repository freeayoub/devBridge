...
import { CallService, Call, CallType, CallStatus } from '../../services/call.service';
...
export class ActiveCallComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    const localStreamSub = this.callService.localStream$.subscribe(stream => {
    ...
    const remoteStreamSub = this.callService.remoteStream$.subscribe(stream => {
  ...
  ngAfterViewInit(): void {
    ...
    const localStream = this.callService.localStream$.getValue();
    if (localStream && this.localVideo?.nativeElement) {
      this.localVideo.nativeElement.srcObject = localStream;
    }
    
    const remoteStream = this.callService.remoteStream$.getValue();
  ...
  endCall(): void {
    ...
    this.callService.endCall(this.activeCall.id).subscribe({
      next: (call) => {
        this.logger.debug('Call ended successfully', { callId: call.id, duration: call.duration });
      },
      error: (error) => {
