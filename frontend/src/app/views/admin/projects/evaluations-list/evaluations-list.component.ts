...
export class EvaluationsListComponent implements OnInit, OnDestroy {
        ...
        next: (evaluations) => {
          ...
          this.evaluations = evaluations.map((evaluation) => {
            ...
            if (
              ...
              console.warn(
                "Détails du projet manquants pour l'évaluation:",
                evalWithDetails._id
              );
  ...
  applyFilters(): void {
    ...
    if (this.searchTerm.trim() !== '') {
      ...
      results = results.filter((evaluation) => {
        ...
        const groupName = this.getStudentGroup(
          evaluation.etudiant
        ).toLowerCase();
  ...
  getStudentInitials(etudiant: any): string {
    ...
    const fullName =
      etudiant.fullName || etudiant.name || etudiant.username || '';
  ...
  getStudentGroup(etudiant: any): string {
    ...
    if (
      etudiant.group &&
      typeof etudiant.group === 'object' &&
      etudiant.group.name
    ) {
      console.log(
        `Groupe objet trouvé pour ${etudiant.email}: ${etudiant.group.name}`
      );
    ...
    if (
      etudiant.group &&
      typeof etudiant.group === 'string' &&
      etudiant.group.trim()
    ) {
      console.log(
        `Groupe string trouvé pour ${etudiant.email}: ${etudiant.group}`
      );
    ...
    if (
      etudiant.groupe &&
      typeof etudiant.groupe === 'string' &&
      etudiant.groupe.trim()
    ) {
      console.log(
        `Groupe (ancien champ) trouvé pour ${etudiant.email}: ${etudiant.groupe}`
      );
    ...
    if (
      etudiant.groupName &&
      typeof etudiant.groupName === 'string' &&
      etudiant.groupName.trim()
    ) {
      console.log(
        `GroupName trouvé pour ${etudiant.email}: ${etudiant.groupName}`
      );
    ...
    if (
      etudiant.department &&
      typeof etudiant.department === 'string' &&
      etudiant.department.trim()
    ) {
      console.log(
        `Department trouvé pour ${etudiant.email}: ${etudiant.department}`
      );
  ...
  getScoreIconClass(score: number): string {
    ...
    if (score >= 12)
      return 'bg-info/10 dark:bg-dark-accent-primary/10 text-info dark:text-dark-accent-primary';
    if (score >= 8)
