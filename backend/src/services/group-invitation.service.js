...
class GroupInvitationService {
   ...
   */
  async createInvitation({ conversationId, inviterId, inviteeId, message = '' }) {
    try {
      ...
      await pubsub.publish(`GROUP_INVITATION_RECEIVED_${inviteeId}`, {
        groupInvitationReceived: this.formatInvitationResponse(populatedInvitation)
      });
  ...
  async getPendingInvitationsForUser(userId) {
    try {
      const invitations = await GroupInvitation.find({
        ...
        .populate('conversationId', 'groupName groupPhoto groupDescription participants')
        .populate('inviterId', 'username email image isOnline lastActive')
        .populate('inviteeId', 'username email image')
        .sort({ createdAt: -1 });
      
      return invitations.map(invitation => this.formatInvitationResponse(invitation));
    } catch (error) {
      console.error(`Error getting pending invitations: ${error.message}`, error);
      throw error;
    }
  }
  ...
  async acceptInvitation(invitationId, userId) {
    try {
      ...
      const conversation = await Conversation.findById(invitation.conversationId._id);
      if (!conversation) {
      ...
      await pubsub.publish(`GROUP_INVITATION_STATUS_CHANGED_${invitationId}`, {
        groupInvitationStatusChanged: this.formatInvitationResponse(populatedInvitation)
      });
  ...
  formatInvitationResponse(invitation) {
    ...
    try {
      ...
      // Formater l'ID
      invitationObj.id = invitationObj._id ? invitationObj._id.toString() : invitationObj.id;
